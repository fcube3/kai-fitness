    ...
    <main className="min-h-screen bg-zinc-950 px-4 py-8 text-zinc-100 sm:px-6">
      <div className="mx-auto w-full max-w-4xl">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <a href="/dashboard" className="text-sm text-emerald-400 hover:underline">← Dashboard</a>
        </div>
                ...
                <div className="border-t border-white/5 px-4 py-3 space-y-1">
                  {s.exercises.map((ex, i) => (
                    <div key={i} className="flex flex-wrap items-baseline gap-2">
                      <a href={`/exercises/${encodeURIComponent(ex.name)}`} className="text-sm font-medium text-zinc-200 hover:text-emerald-400">{ex.name}</a>
